...
import { Survey, CreateSurveyData, CreateSurveyQuestionData } from '@/types/survey';
  ...
  const fetchSurvey = useCallback(async () => {
    try {
      ...
      setFormData({
        ...
        daysAfterHiring: survey.daysAfterHiring,
      ...
      if (survey.questions && survey.questions.length > 0) {
        const sortedQuestions = survey.questions
          ...
          .map(q => ({
            id: q.id, // Keep the ID for updates
            question: q.question,
            order: q.order,
      ...
      toast.error('Error al cargar la encuesta', {
        description: 'No se pudo cargar la información de la encuesta',
        duration: 5000
      });
    ...
    return (
      <div className="p-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl font-bold">Cargando encuesta...</h1>
        </div>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button type="submit" disabled={loading}>
            {loading ? 'Actualizando...' : 'Actualizar Encuesta'}
          </Button>
        </div>
      </form>
    </div>
